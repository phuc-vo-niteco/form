...
export default (item) => {
    ...
    const [optionSource, setOptionSource] = useState(Array.from(value || [
        {
            value: '',
            lable: '',
        }
                ...
                </div>
                {
                    ...
                    (<div key={index} className="field-group-item">
                        <input
                            ...
                            placeholder="option"
                            value={option.value}
                            onChange={(event) => onChangeItem({ event, index, key: 'value' })}
                        />
                        <input
                            className="form-control"
                            placeholder="Option"
                            value={option.label}
                            onChange={(event) => onChangeItem({ event, index, key: 'label' })}
                        />
                        <button disabled={optionSource.length <= 1} onClick={() => removeItem(index)} className="field-group-action field-group-remove btn">
                            <Icons.DashCircle />
                        </button>
                        <button onClick={addItem} className="field-group-action field-group-add btn">
                            <Icons.PlusCircle />
                        </button>
                    </div>))
                }
